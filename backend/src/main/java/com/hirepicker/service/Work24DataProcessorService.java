...
public class Work24DataProcessorService {
    ...
    public void processCompanyDto(CompanyDto dto) {
        companyRepository.findByCompanyId(dto.id()).ifPresentOrElse(
            ...
            () -> companyRepository.save(Company.builder().companyId(dto.id()).companyName(dto.name()).description(dto.summary()).websiteUrl(dto.homepage()).businessNumber(dto.businessNumber()).logoUrl(dto.logoUrl()).companyType(dto.companyType()).build()) // 새로운 기업이면 저장
        );
    }
}
