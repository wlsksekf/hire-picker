...
public interface PostRepository extends JpaRepository<Posts, Long>{
   ...
   @Query("SELECT new com.hirepicker.dto.PostListDto(p.postIdx, u.nickname, p.title, p.content, p.createdAt, p.viewCount) " +
       "FROM Posts p JOIN PersonalUser u ON p.pUserIdx = u.id " +
       "WHERE p.boardIdx = :boardIdx")
Page<PostListDto> findPostListWithNickname(@Param("boardIdx") Long boardIdx, Pageable pageable); // ← @Param!
