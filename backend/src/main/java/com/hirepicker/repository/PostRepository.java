...
@Query("SELECT new com.hirepicker.dto.PostListDto(p.postIdx, p.personalUser.id, u.nickname, p.title, p.content, p.createdAt, p.viewCount, p.imgName, p.fileName) " +
       "FROM Posts p JOIN PersonalUser u ON p.personalUser.id = u.id WHERE p.boardIdx = :boardIdx")
...
@Query("SELECT new com.hirepicker.dto.PostListDto(" +
       "p.postIdx, p.personalUser.id, u.nickname, p.title, p.content, p.createdAt, p.viewCount, p.imgName, p.fileName) " +
       "FROM Posts p JOIN PersonalUser u ON p.personalUser.id = u.id " +
