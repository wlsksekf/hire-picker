...
export default function SearchFilterBar({ onSearchAndFilter }) {
  ...
  const [filters, setFilters] = useState({           // 선택된 필터 상태
    jobType: [],
    location: [],
    employmentType: [],
    experienceLevel: [],
  ...
  function handleFilterChange(category, value) {
    setFilters(function(prev) {
      ...
      const updated = current.includes(value)
        ? current.filter(function(v) { return v !== value; }) // 제거
  ...
  function handleSearch() {
    axios.post(`/api/search?page=0&size=${PAGE_SIZE}`, {
  ...
  function handleResetAll() {
    const empty = { jobType: [], location: [], employmentType: [], experienceLevel: [], companyType: [] };
    setSearchTerm('');
    setFilters(empty);
  ...
  return (
    <Box sx={{ mb: 4 }}>
      ...
      <Box sx={{ maxWidth: '700px', mx: 'auto', mb: 4 }}>
        <TextField
          ...
          InputProps={{
            ...
            endAdornment: (
              <IconButton onClick={handleSearch}> {/* 돋보기 클릭 */}
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M11 19C15.4 19 19 15.4 19 11C19 6.6 15.4 3 11 3C6.6 3 3 6.6 3 11C3 15.4 6.6 19 11 19Z" stroke="#333" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      ...
      <Box sx={{ mb: 3, display: 'flex', flexWrap: 'wrap', gap: 1.5, alignItems: 'center' }}>
        {[
          ...
          return (
            <Button
              ...
              onClick={function(e) { handleOpenPopover(e, key); }}
              sx={{ borderRadius: '8px' }}
            >
        ...
        {/* 전체 초기화 버튼 */}
        <Button startIcon={<FontAwesomeIcon icon={faRedo} size="sm" />} onClick={handleResetAll} sx={{ color: 'text.secondary' }}>
          초기화
        </Button>
      </Box>
      ...
      <Popover
        ...
        PaperProps={{
          sx: { width: '500px', maxHeight: '400px', borderRadius: '12px', p: 2, mt: 1 },
        }}
      >
        {/* 팝업 헤더 */}
        ...
        <FormGroup sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0.5 }}>
          {currentFilterType && getFilterOptions(currentFilterType).map(function(option) {
            return (
              <FormControlLabel
                key={option}
                control={
                  <Checkbox
                    ...
                    onChange={function() { handleFilterChange(currentFilterType, option); }}
                    size="small"
                  />
                }
                label={option}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
          <Button onClick={function() { handleResetFilters(currentFilterType); }}>초기화</Button>
